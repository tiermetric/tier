...
| Codex CLI (OpenAI)      | Yes — rollout logs ¶      | **No** — Responses API, unreadable by the proxy ¶ | Yes | **Works in v1** (`collectors.codex_rollout` / `--codex-rollout`, #464) |
| OpenAI / xAI / DeepSeek | n/a                       | Yes (JSON; SSE on #14) †  | Yes         | **Works in v1**     |
| Google Gemini           | n/a                       | No route — parser only ‡  | Yes         | **Not capturable via proxy** (parser present, unrouted) |
...
>
> **‡ Gemini is not captured today.** A `parseGemini` unmarshaller exists (`internal/proxy/proxy.go`), but `tierd serve` mounts **only** `/anthropic/` and `/openai/` (`cmd/tierd/main.go`) — there is no `/gemini/` route or flag, so live Gemini API traffic is **not capturable via the proxy**. The parser is dormant until a route is wired. (Manual REST `POST /api/v1/costs` can still import Gemini spend.)
>
...
>
> **¶ Codex is captured from local logs, and only from local logs.** The proxy **cannot** capture Codex: Codex speaks the OpenAI **Responses** API (`input_tokens` / `output_tokens`), while `parseOpenAI` reads the **Chat Completions** usage shape (`prompt_tokens` / `completion_tokens`), so a Codex response routed through `/openai/` yields **no** `TokenEvent` at all (#463). That gap is closed from the other side: Codex writes per-session rollout logs to `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl`, and `collectors.codex_rollout` (or `--codex-rollout`) reads them directly — local, per-developer, per-call, no credential and no env-var change (#464). Routing Codex through the proxy remains pointless until a Responses-API parser is wired; the local path is the supported one.
...
**Why everything else needs the proxy.** Cursor (when using API keys) and any custom OpenAI/Anthropic SDK consumer don't emit local logs. To capture them you point `ANTHROPIC_BASE_URL` / `OPENAI_BASE_URL` at `tierd serve`, which acts as a transparent reverse proxy and reads `usage` blocks off the wire. (Gemini CLI traffic is **not** capturable this way today — the Gemini response parser exists but no `/gemini/` route is mounted; see the ‡ note above. Codex is **not** capturable this way either; see ¶.)
...
- **This is the only Codex capture path.** See the ¶ note in §2. **Both** proxy paths read Chat Completions and neither reads Responses: `parseOpenAI` on the JSON path, and `openAIStreamParser` (`internal/proxy/sse.go`) on the SSE path Codex actually uses, since Codex streams by default and Responses SSE nests usage under the response object. Proxying Codex therefore captures nothing (#463).
...
### 3c. Reverse proxy (OpenAI / Claude API direct)

- Entry point: `proxy.New` at `internal/proxy/proxy.go`; mounted under **only** `/anthropic/` and `/openai/` by `tierd serve` (`cmd/tierd/main.go`). There is **no** `/gemini/` route.
- **JSON path**: `handleJSON`. Buffers the response (10 MB cap), parses with the per-provider unmarshaller, emits one `TokenEvent`. Two mounted parsers, plus one dormant:
  - `parseAnthropic` — `msg_*` id, `usage.input_tokens` / `output_tokens` / cache fields. **Routed** (`/anthropic/`).
  - `parseOpenAI` — `chatcmpl-*` id, `usage.prompt_tokens` / `completion_tokens`, optional `prompt_tokens_details.cached_tokens`. **Routed** (`/openai/`). Chat Completions **only** — the Responses shape (`usage.input_tokens` / `output_tokens`) is unparsed here and in `openAIStreamParser`, which is why Codex needs the rollout-log collector (§3b).
  - `parseGemini` — `responseId`, `usageMetadata.promptTokenCount` / `candidatesTokenCount`. **Present but NOT routed**: the unmarshaller is written and unit-tested, but no `/gemini/` mount or flag exists in `cmd/tierd`, so no live Gemini traffic ever reaches it. Gemini capture is a wiring task, not a shipped path.
...
- Reverse proxy captures Anthropic / OpenAI JSON responses (SSE streaming, PR #14, merged). Gemini has a response parser but **no mounted route**, so Gemini traffic is not captured today.
